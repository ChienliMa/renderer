...
def random_blend( top, base, top_alpha = None , base_alpha = None, 
    ...
    if modes == None:
        modes = all_modes()
    modes = modes.split()
    # tempary sulotion for better rendering, consider it for a feature
    modes.remove('normal')
    mode = modes[np.random.randint(0,len(modes))]
    return blend( top, base, mode , top_alpha = None, 
                    base_alpha = None, mask = mask )
